...
def img(pair):
   ...
   data_YD = web.DataReader('GBPJPY=X', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='GBPJPY')
   plt.title("GBPJPY")
   plt.xlabel('date')
   plt.ylabel('GBPJPY')
   plt.savefig("../../img/GBPJPY.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('GBPJPY', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'GBPJPY'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "GBPJPY=X" でも同じ。 開始日/終了日は date型でも指定可能
