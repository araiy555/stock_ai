...
def img(pair):
   ...
   data_YD = web.DataReader('^NDX', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='NDX')
   plt.title("NDX")
   plt.xlabel('date')
   plt.ylabel('NDX')
   plt.savefig("../../img/NDX.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('NDX', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'NDX'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "NDX=X" でも同じ。 開始日/終了日は date型でも指定可能
