...
def img(pair):
   ...
   data_YD = web.DataReader('AUDJPY=X', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='AUDJPY')
   plt.title("AUDJPY")
   plt.xlabel('date')
   plt.ylabel('AUDJPY')
   plt.savefig("../../img/AUDJPY.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('AUDJPY', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'AUDJPY'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "AUDJPY=X" でも同じ。 開始日/終了日は date型でも指定可能
