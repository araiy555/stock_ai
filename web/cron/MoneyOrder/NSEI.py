...
def img(pair):
   ...
   data_YD = web.DataReader('^NSEI', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='NSEI')
   plt.title("NSEI")
   plt.xlabel('date')
   plt.ylabel('NSEI')
   plt.savefig("../../img/NSEI.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('NSEI', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'NSEI'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "NSEI=X" でも同じ。 開始日/終了日は date型でも指定可能
