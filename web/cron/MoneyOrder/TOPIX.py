...
def img(pair):
   ...
   data_YD = web.DataReader('^TOPX', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='TOPX')
   plt.title("TOPX")
   plt.xlabel('date')
   plt.ylabel('TOPX')
   plt.savefig("../../img/TOPX.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('TOPX', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'TOPX'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "TOPX=X" でも同じ。 開始日/終了日は date型でも指定可能
