...
def img(pair):
   ...
   data_YD = web.DataReader('^HSI', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='HSI')
   plt.title("HSI")
   plt.xlabel('date')
   plt.ylabel('HSI')
   plt.savefig("../../img/HSI.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('HSI', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'HSI'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "HSI=X" でも同じ。 開始日/終了日は date型でも指定可能
