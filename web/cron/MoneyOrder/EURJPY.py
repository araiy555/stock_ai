...
def img(pair):
   ...
   data_YD = web.DataReader('EURJPY=X', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='EURJPY')
   plt.title("EURJPY")
   plt.xlabel('date')
   plt.ylabel('EURJPY')
   plt.savefig("../../img/EURJPY.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('EURJPY', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'EURJPY'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "EURJPY=X" でも同じ。 開始日/終了日は date型でも指定可能
