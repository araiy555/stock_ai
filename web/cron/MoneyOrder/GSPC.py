...
def img(pair):
   ...
   data_YD = web.DataReader('^GSPC', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='GSPC')
   plt.title("GSPC")
   plt.xlabel('date')
   plt.ylabel('GSPC')
   plt.savefig("../../img/GSPC.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('GSPC', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'GSPC'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "GSPC=X" でも同じ。 開始日/終了日は date型でも指定可能
