...
def img(pair):
   ...
   data_YD = web.DataReader('^VIX', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='VIX')
   plt.title("VIX")
   plt.xlabel('date')
   plt.ylabel('VIX')
   plt.savefig("../../img/VIX.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('VIX', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'VIX'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "VIX=X" でも同じ。 開始日/終了日は date型でも指定可能
