...
def img(pair):
   ...
   data_YD = web.DataReader('^RUT', 'yahoo', start, end) #get data

   plt.plot(data_YD['Adj Close'],label='RUT')
   plt.title("RUT")
   plt.xlabel('date')
   plt.ylabel('RUT')
   plt.savefig("../../img/RUT.png")
   plt.legend()
   plt.show()
...
if result['count'] == 0:
    ...
    cursor.execute(sql, ('RUT', TODAY, WEEK))
    conn.commit()
else:
    sql = ('UPDATE moneyorder SET today = %s, week = %s , create_at = %s WHERE code_name = %s')
    cursor.execute(sql, (TODAY, WEEK, time.strftime('%Y-%m-%d %H:%M:%S'), 'RUT'))
...
# -----------------------------------------------------------------------------------
# Symble指定は "JPY=X" でも "RUT=X" でも同じ。 開始日/終了日は date型でも指定可能
