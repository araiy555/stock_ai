...
def setCsv(row):
    try:
        indx = row['symbol'].replace('.INDX', '')
        indx = '^' + indx
       
        df = web.DataReader(indx, "yahoo")
...
with conn.cursor() as cursor:
    sql = "SELECT * FROM marketstock WHERE mic = 'INDX'"
    cursor.execute(sql)
...
##df_ntt = web.DataReader('7203.T',"yahoo") indx
